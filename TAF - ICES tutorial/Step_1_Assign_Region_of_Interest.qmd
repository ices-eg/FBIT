---
title: "Building a Regional Grid"
...
This step demonstrates how to create a spatial grid over a region of interest, assign an unqiue ID to the grid, and enrich the grid with environmental data such as depth and seabed stress.

We will predominantly use the R-packages sf and ggplot2 to build and visualize the dataset. The spatial grid is created for the Southern North Sea, corresponding to the data used in Step 2.
...
We define the origin (lower-left corner), cell size, and number of cells to cover the area of interest.
...
C-squares are a spatial indexing system. We assign them to the grid cells.
...
We now load environmental data (depth and habitat type) and match it to the grid based on the 0.05 c-square ID.
...
## Adding Other Environmental Data

Environmental data you add to the grid may differ in file type, grid resolution, and spatial or temporal coverage. Integrating these datasets into the grid often requires some adjustment. As an example, we will add seabed stress to the grid cell using the nearest mid-point.
...
ggplot() +
  geom_sf(data = grid_sf, aes(col = bedstress, fill=bedstress)) +
...
Obtain an environmental data layer for the southern North Sea from an online source, and then add this layer to your grid dataset.
