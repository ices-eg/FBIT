...
The benthic data comes from a Dutch monitoring program carried out in 2007. Samples were collected using a box corer, with one sample taken at each location. Because the number of stations is relatively low, the reliability of the predictions is limited. The analysis we aim to perform is therefore mainly illustrative and serves as an example.
...
We plot the stations within the grid defined in Step 1. As you can see, the number of stations is limited (but adequate to illustrate the methodology).

```{r}
load("model/Region_grid.RData")

ggplot() +
  geom_sf(data = grid_sf, aes(col = Depth, fill=Depth)) +
  ggtitle("Depth across grid cells") +
  geom_point(data=statEnv, aes(x=poslon, y=poslat),col="red")
...
The goal of the statistical model is to estimate which environmental conditions drive variation in the cumulative biomass-longevity distributions. For example, if benthic fauna tend to be longer-lived in deeper waters, we want to quantify this relationship to predict how longevity, and therefore sensitivity, varies with depth.

The statistical model we are typically using has the following structure:
...
For this example, we examine three environmental data layers: depth, sediment type (categorical), and bed shear stress. To obtain sediment type information, we use the MSFD broad habitat type classification, which is derived from both depth and sediment characteristics. We do not use the depth information from the MSFD classification, since depth can also be presented as a continuous variable, which is generally more suitable for model fitting.

```{r}

# Assign sediment categories from MSFD broad habitat types
...
Stress    <- log(rep(statEnv$Bedstress, 3))
Cumb      <- c(statEnv$L1, statEnv$L1 + statEnv$L1_3, 
               statEnv$L1 + statEnv$L1_3 + statEnv$L3_10)
Longevity <- c(rep(1, nrow(statEnv)), rep(3, nrow(statEnv)), 
               rep(10, nrow(statEnv)))

fulldat <- data.frame(ID, Sediment, Depth, Cumb, Longevity, Stress)
fulldat$ll <- log(fulldat$Longevity)

# Values need to be between 0 and 1, update "extremes"
...
mod2 <- glmer(Cumb ~ ll + Sediment + Depth + (1 | ID), 
              data=fulldat, family=binomial)
mod3 <- glmer(Cumb ~ ll + Sediment + Depth + Stress + (1 | ID), 
              data=fulldat, family=binomial)
mod4 <- glmer(Cumb ~ ll + Depth + Stress + (1 | ID), 
              data=fulldat, family=binomial)
mod5 <- glmer(Cumb ~ ll + Sediment + (1 | ID), 
              data=fulldat, family=binomial)

# Compare AIC
AIC(mod1, mod2, mod3, mod4,mod5)

# a model with only sediment is the best model
# for the purpose of illustrating, we use model 2

```

## Save the fixed effects of the model

Finally, we save the fixed effects of the model as an object for later use.
...
Match the environmental data layer obtained in Step 1 with the sampling locations. Incorporate this environmental information into your statistical model and repeat the model selection process.
