...
# depletion rates is based on  Hiddink et al. PNAS 2017 / Rijnsdorp et al. ICES 2020
  ...
  ### calculate state for all gears
    ...
    state_sens <- rep(1,nrow(loopdata))
    
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_tot[j] > 0 & !(is.na(loopdata$intercept[j]))) {
        state[j]      <- RBS(Fd=loopdata$Depl_tot[j],a=loopdata$slope[j],b=loopdata$intercept[j],H=H)
        state_sens[j] <- RBS_sens(Fd=loopdata$Depl_tot[j],a=loopdata$slope[j],b=loopdata$intercept[j],H=H)
      }}
    
    state_year <- cbind(state_year,state)
    state_sens_year <- cbind(state_sens_year,state_sens)
    
    ccname <- c(ccname,paste("state",Period[i],sep="_"))
    ccname_sens <- c(ccname_sens,paste("state_sens",Period[i],sep="_"))
  ...
  colnames(state_sens_year) <- ccname_sens
